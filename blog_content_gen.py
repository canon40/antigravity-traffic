...
def _get_keyword_and_scope_instructions(config, required_keyword, extra_keyword):
    ...
    if any(k in kw_all for k in ["욕실", "화장실", "샤워부스", "싱크", "주방", "대리석", "가구", "원목", "마루", "테이블", "식탁", "리빙"]):
        # 리빙/생활용 코팅 제품군 (듀라코트) - 다양한 장면 무작위 선택
        if any(k in kw_all for k in ["욕실", "화장실", "샤워부스", "타일"]):
            bathroom_scenes = [
                "bright bathroom interior with tiles and glass shower booth, water beads visible on surfaces",
                "close-up of water beading on a transparent glass shower screen, clean modern bathroom background",
                "luxury marble bathroom sink with premium metallic faucet, water droplets forming round beads",
                "modern grey tiled bathroom floor and wall, clean protective coating look, soft diffused light"
            ]
            scene_hint = random.choice(bathroom_scenes)
        elif any(k in kw_all for k in ["주방", "싱크", "싱크대", "대리석"]):
            kitchen_scenes = [
                "modern kitchen sink and marble countertop area, clean polished surfaces, trace of water beading",
                "brushed stainless steel kitchen sink with perfect water beading, clean and neat",
                "clean white quartz kitchen island countertop reflecting soft daylight, modern home interior",
                "glossy black induction cooktop reflecting clean lights, pristine kitchen countertop"
            ]
            scene_hint = random.choice(kitchen_scenes)
        else:
            furniture_scenes = [
                "wooden table and furniture in a living room, warm lighting, clean glossy coated surface",
                "rustic solid wood dining table in a sunlit room, clean protective coat highlighting the grain",
                "modern dark walnut coffee table with glossy protective surface reflecting living room lights",
                "close-up of a wooden cabinet surface showing clean wood grain and deep glossy protective coat"
            ]
            scene_hint = random.choice(furniture_scenes)
    elif any(k in kw_all for k in ["오토바이", "바이크", "헬멧", "bike", "motorcycle"]):
        # 바이크 코팅 제품군 - 다양한 장면 무작위 선택
        bike_scenes = [
            "motorcycle tank and engine area, metallic gloss and water-repellent coating effect",
            "a high-end motorcycle parked on a scenic mountain road during golden hour, reflecting warm sun",
            "close-up of a glossy black motorcycle helmet resting on a seat, showing clean polished reflection",
            "metallic motorcycle exhaust pipe and wheel rim shining under bright studio lights, deep polish"
        ]
        scene_hint = random.choice(bike_scenes)
    elif any(k in kw_all for k in ["자동차", "카", "차량", "유리막", "퍼마코트", "발수", "세차", "광택", "car", "auto", "vehicle"]):
        # 자동차 코팅 제품군 (퍼마코트) - 다양한 장면 무작위 선택
        car_scenes = [
            "luxury car body and hood close-up, deep gloss reflection, strong water beading on coated paint",
            "a sleek sports car parked in a modern concrete garage, glossy mirror-like paint reflection",
            "close-up of a car door mirror and clean window showing beautiful round water beading",
            "sports car hood reflecting city neon lights at night, deep glossy paint coating showcase",
            "a glossy black sedan parked on a scenic coastal road during a beautiful sunset, polished paint"
        ]
        scene_hint = random.choice(car_scenes)
    else:
        # 기본값: 자동차 관련 다양한 장면 무작위 선택
        default_scenes = [
            "car body close-up with deep glossy reflection and perfect water beading on paint",
            "car hood reflecting blue sky and clouds, pristine glossy coating showcase",
            "close-up of shiny alloy wheels and clean side profile of a luxury car, glossy finish",
            "modern car window and side door panel with clear hydrophobic water repellent beads"
        ]
        scene_hint = random.choice(default_scenes)
